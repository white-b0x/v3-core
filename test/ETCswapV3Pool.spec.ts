...
import { TestETCswapV3SwapPay } from '../typechain/TestETCswapV3SwapPay'
...
import { TestETCswapV3ReentrantCallee } from '../typechain/TestETCswapV3ReentrantCallee'
...
describe('ETCswapV3Pool', () => {
  ...
  let factory: ETCswapV3Factory
  let pool: MockTimeETCswapV3Pool

  let swapTarget: TestETCswapV3Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeETCswapV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestETCswapV3ReentrantCallee')
      ).deploy()) as TestETCswapV3ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestETCswapV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestETCswapV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestETCswapV3SwapPay
